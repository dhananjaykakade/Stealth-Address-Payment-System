...
**Positioning:** This project demonstrates how stealth payments can be delivered through **real wallet infrastructure** (BitGo SDK + MPC custody), not just cryptographic theory. It’s designed to be directly relevant to **institutional custody and wallet platforms** where privacy, auditability, and operational safety must coexist.
...
The Stealth Address Payment System enables **private, unlinkable payments** on the blockchain while keeping the user experience familiar to modern wallet products. Instead of sending funds directly to a receiver’s static address, the sender derives a **one-time stealth address** from the receiver’s published public keys. This ensures:

- **Receiver anonymity** — observers cannot link multiple payments to the same receiver
- **Unique addresses per payment** — each transaction uses a fresh on-chain destination
- **Reusable public identity** — the receiver shares one stealth meta-address that remains valid long-term
- **On-chain verifiability** — transactions remain fully verifiable; privacy comes from unlinkability, not obscurity

At its core, this system implements the [ERC-5564](https://eips.ethereum.org/EIPS/eip-5564) and [ERC-6538](https://eips.ethereum.org/EIPS/eip-6538) standards using the [`@scopelift/stealth-address-sdk`](https://github.com/ScopeLift/stealth-address-sdk), and pairs it with **BitGo MPC wallet infrastructure** to make stealth payments practical in real custody workflows.
